...
$(function() {
/*	var publicClassVal=$('input:radio[name="publicclass"]:checked').val();
	if(publicClassVal==1){
	...
	var buttonStart = $.extend([], $.fn.datetimebox.defaults.buttons);
	buttonStart.splice(2, 0, {
		text: '清空',
		handler: function(target){
			$('#studyTimeStart').datetimebox('setValue', ''); 
		}
	});
	var buttonEnd = $.extend([], $.fn.datetimebox.defaults.buttons);
	buttonEnd.splice(2, 0, {
		text: '清空',
		handler: function(target){
			$('#studyTimeEnd').datetimebox('setValue', ''); 
		}
	});
	$('#studyTimeStart').datetimebox({
		buttons: buttonStart
	});
	$('#studyTimeEnd').datetimebox({
		buttons: buttonEnd
	});
	// 课程分类
	...
	//用户列表
	$.post(baseUrl + '/admin/C010/findSelectedUser.html',{"cid": $("#courseId").val()},function(data){
		var userOrderProgres= data.userOrderProgres;
		var userList=data.userList;
		var jsonStr=JSON.stringify(userOrderProgres);
		$('#userOrderProgres').val(jsonStr);
		$('#addedUserList').datagrid("loadData",userList);
	},"json");
	//试卷列表
	$.ajax({
		url:baseUrl + '/admin/findShiJuanSelected.html',
		data:{'shiJuanIds': sjStr},
		type: 'post',
		dataType: 'json',
		success:function(result){
			$("#selectExamPaperList").datagrid({"data":result});
		}
	});

	//附件列表
	$.ajax({
		url:baseUrl + '/admin/findAccessoriesSelected.html',
		data:{'accessoriesIds': fjStr},
		type: 'post',
		dataType: 'json',
		success:function(result){
			$("#selectedAccessoriesList").datagrid({"data":result});
		}
	});

	//证书列表
	$.ajax({
		url:baseUrl + '/admin/findCertSelected.html',
		data:{'certIds': certStr},
		type: 'post',
		dataType: 'json',
		success:function(result){
			$("#selectedCertificatesList").datagrid({"data":result});
		}
	});
	//添加表单加载验证
	$('#updForm').form({
		url : "updCourse.html",
		onSubmit : function() {
			var examPaper = $('#selectExamPaperList').datagrid('getRows');
		...
		success : function(data) {
			if(data>0){
				$.messager.confirm('提示', '更新课程成功！', function(r) {
					if (r) {
...
//试卷列表(已选的)
function creatExamPaperGrid(ids) {
...
//附件列表
function creatUnSelectedAccessoriesGrid(idAttr) {
	$('#unSelectedAccessoriesList').datagrid({
		queryParams: {
			'courseAccessoriesIds': idAttr
		},
		fit : true,// 设置为true时铺满它所在的容器.
		fitColumns : true,// 设置为true将自动使列适应表格宽度以防止出现水平滚动
		rownumbers:true,// 行号
...
//证书列表未选择
function creatUnSelectedCertificatesGrid(idAttr) {
...
function openExamShiJuanWin(){
	...
	var ids=idAttr.substring(0,idAttr.lastIndexOf(","));
	
	if(shiJuanFirstLoadflg==0){
		creatExamPaperGrid(ids);
...
function openAccessoriesWin(){
	...
	if(accessoriesFirstLoadflg==0){
		...
		creatUnSelectedAccessoriesGrid(idAttr);
...
function openCertificatesWin(){
	...
	if(certificatesFirstLoadFlg==0){
		...
		creatUnSelectedCertificatesGrid(idAttr);
...
function saveCourseware(){
	...
	if( typeof($("#"+id).find(".courseware").children().html())=="undefined"){
		...
		courseStr+='<div style="width:100%"><table cellspacing="0" cellpadding="0" border="0" style="width:510px;border-bottom: 1px solid #ccc;">';
...
//		courseStr+='</tbody>';
	...
	var course="";
	var userOrderProgres=$("#userOrderProgres").val();
	var data = eval('(' + userOrderProgres + ')');
	for(var i=0;i<items.length;i++){
		fmtImg(items[i].typeNum);
		course+='<tr>';
		course+='<td width="100px"><input name="coursewareTime" value="'+items[i].timelength+'" type="hidden"/><input name="cid" value="'+items[i].coursewareId+'" type="hidden"/><input class="coursewareId" name="'+id+'_coursewareId" value="'+items[i].coursewareId+'" type="hidden"/><img width="37" height="34" src="'+imgStr+'" /></td>';
		...
		course+='<td width="60px"><a cwid="'+items[i].coursewareId+'" class="delCoursewareBtn" onclick="delCourseware(this)">删除</a></td>';
		course+='</tr>';
		for(var index in data){
			var key=items[i].coursewareId+"";
			var test=eval("("+data[index]+")");
			test[key]=0;
			data[index]=JSON.stringify(test);
		}
	}
	$("#userOrderProgres").val(JSON.stringify(data));
...
function delSection(obj){
	var prevParentObj=$(obj).parent().parent().prev().parent();
	$(obj).parent().parent().remove();
	prevParentObj.find(".sectionName").each(function(i){
		$(this).text("第"+(i+1)+"节：");
		$(this).parent().parent().attr("sectionId",(i+1));
	});
	//计算学时
	calculateStudyTime();
}
...
function fmtExamJson(){
	...
	if($.trim(examName)!=""){
		...
		// 答题模式
		var pageSize=$("#pageSize").parent().find(".textbox-value").val();
		// 考试次数
		var maxTimes=$("#maxTimes").parent().find(".textbox-value").val();
		// 及格后不能再考
...
/*	$('#addedUserList').datagrid({
	...
	$.ajax({
		url:baseUrl + '/admin/U010/findUserForLearn.html',
		data:{'groupids': groupidstr,'userids':userIdStr},
		type: 'post',
		dataType: 'json',
		success:function(result){
			var rows=result.rows;
			var userOrderProgres=eval("("+$("#userOrderProgres").val()+")");
			for(var i=0;i<rows.length;i++){
				var userKey=rows[i].userid+"";
				userOrderProgres[userKey]=coursewareJson;
				$('#addedUserList').datagrid("appendRow",rows[i]);
			}
			$("#userOrderProgres").val(JSON.stringify(userOrderProgres));
		}
	});
...
function deleteUser(){
	...
	for(var i in rows){
		var userKey=rows[i].userid+"";
		delete userOrderProgres[userKey];
...
//课程Json解析
function courseAnalyzeJson(){
	var data = eval('(' + coursewareJsonStr + ')');

	for(var i=0;i<data.length;i++){
		var chapterHtml="";
		//章名称
		var chapterName=data[i].chapterName;
		chapterHtml+='<div id="chapter_'+(i+1)+'">';
		chapterHtml+='<div class="formItem borderDotted chapter">';
		chapterHtml+='<div style="margin-left:25px;" class="formField">';
		chapterHtml+='<span style="font-size: 13px;" class="chapterName" >第'+(i+1)+'章：</span>';
		chapterHtml+='<input name="chapter" type="text" value="'+chapterName+'" class="easyui-validatebox input_course chapter_input" data-options="required:true" style="width:350px;height:28px;"/>';
		chapterHtml+='<a style="margin-left:20px;" class="easyui-linkbutton addSectionBtn" id="'+(i+1)+'" onclick="addSection(this)" >添加节</a>';
		if(i!=0){
			chapterHtml+='<a class="delSectionBtn" onclick="delChapter(this)">删除</a>';
		}
		chapterHtml+='</div>';
		chapterHtml+='</div>';
		chapterHtml+='</div>';
		$(".pageContainerBox").append(chapterHtml);

		for(var j=0;j<data[i].section.length;j++){
			var id="section_"+(i+1)+"_"+(j+1);
			//节
			//节名称
			var sectionHtml="";
			var sectionName=data[i].section[j].sectionName;
			sectionHtml+='<div id="section_'+(i+1)+'_'+(j+1)+'" sectionId="'+(j+1)+'" class="formItem Section moseoverYellow borderDotted section">';
			sectionHtml+='<div style="margin-left:50px;" class="formField">';
			sectionHtml+='<span style="font-size: 13px;" class="sectionName">第'+(j+1)+'节：</span>';
			sectionHtml+='<input name="chapter'+(i+1)+'_section" value="'+sectionName+'" type="text" class="easyui-validatebox input_course section_input" data-options="required:true" style="width:350px;height:28px;"/>';
			sectionHtml+='<a style="margin-left:20px;" class="addCoursewarBtn" cid="section_'+(i+1)+'_'+(j+1)+'" onclick="openCoursewareWin(this)">添加课件</a>';
			if(j!=0){
				sectionHtml+='<a class="delSectionBtn" onclick="delSection(this)">删除</a>';
			}
			sectionHtml+='<div class="courseware1">';
			sectionHtml+='<h3>课件</h3>';
			sectionHtml+='<div class="courseware">';
			sectionHtml+='<div style="width:100%;padding:0px;"><table cellspacing="0" cellpadding="0" border="0" style="width:510px;border-bottom: 1px solid #ccc;">';
//			sectionHtml+='<thead style="width:510px;display:block">';
			sectionHtml+='<tr>';
			sectionHtml+='<td width="100px">课件类型</td>';//类型
			sectionHtml+='<td width="150px">课件名称</td>';//课件名称
			sectionHtml+='<td width="60px">文件大小</td>';//文件大小
			sectionHtml+='<td width="94px">上传日期</td>';//上传日期
			sectionHtml+='<td width="60px">操作</td>';
			sectionHtml+='</tr>';
//			sectionHtml+='</thead>';
			sectionHtml+='</table></div>';
			sectionHtml+='<div style="width:100%;height:135px;overflow-y: auto;padding:0px;"><table cellspacing="0" cellpadding="0" border="0" id="'+id+'_tbody" style="width:510px;">';
//			sectionHtml+='</tbody>';
			sectionHtml+='</table></div>';
			sectionHtml+='</div>';
			sectionHtml+='</div>';
			sectionHtml+='</div>';
			sectionHtml+='</div>';
			$("#chapter_"+(i+1)).append(sectionHtml);
			$("#"+id).find(".courseware1").attr("style","margin-left:50px;height:230px;width:530px;margin-top:50px; border: 1px solid #ccc;padding:10px;");
			//课件
			var kejian=data[i].section[j].kejian;
			var kejianIdStr='';
			for(var k=0;k<kejian.length;k++){
				kejianIdStr+=kejian[k]+""+",";
			}
			var kejianIds = kejianIdStr.substring(0,kejianIdStr.length-1);
		
			//课件列表
			$.ajax({
				url:baseUrl + '/admin/C010/findCoursewares.html',
				data:{'kejianIds': kejianIds},
				async:false,
				type: 'post',
				dataType: 'json',
				success:function(result){
					for(var c=0;c<result.length;c++){
						fmtImg(result[c].type);
						var course="";
						course+='<tr>';
						course+='<td width="100px"><input name="coursewareTime" value="'+result[c].timeLength+'" type="hidden"/><input name="cid" value="'+result[c].coursewareId+'" type="hidden"/><input class="coursewareId" name="'+id+'_coursewareId" value="'+result[c].coursewareId+'" type="hidden"/><img width="37" height="34" src="'+imgStr+'" /></td>';
						course+='<td width="150px">'+result[c].name+'</td>';
						course+='<td width="60px">'+result[c].size+'</td>';
						course+='<td width="94px">'+result[c].updDate+'</td>';
						course+='<td width="60px"><a cwid="'+result[c].coursewareId+'" class="delCoursewareBtn" onclick="delCourseware(this)">删除</a></td>';
						course+='</tr>';
						$("#"+id+'_tbody').append(course);
					}
				}
		...
		$('.delSectionBtn,.delCoursewareBtn').linkbutton({
			iconCls : 'icon-no',
			plain:true
		});
		$(".chapter_input,.section_input").validatebox({
			required: true
		});
}
/**
