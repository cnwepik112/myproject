$(function() {
	...
	$('#updCoursewareFrom').form({
		url : "updCoursewareInfo.html",
		onSubmit : function() {
			return $('#updCoursewareFrom').form("validate");
		},
		success : function(data) {
			if (data > 0) {
				$.messager.confirm('提示', '更新课件信息成功！', function(r) {
					if (r) {
	...
	if($("#coursewareType").val()<5){
		//视频类
		$("#editStudyTime").attr("readonly","readonly");
		$("#editStudyTime").attr("onfocus","this.blur();");
	}
	if($("#coursewareType").val()==5){
		//图片类
		creatPicGrid($("#editCoursewareId").val());
	}
});
...
function creatPicGrid(id) {
	$('#picList').datagrid({
		...
		rownumbers : true,// 行号
		singleSelect:true,
		url : 'findContent.html?coursewareId='+id,
		columns : [ [ {
			field : 'picPath',
			title : '图片',
			width : 80,
...
function submitUpdForm() {
	...
	$.messager.confirm('编辑提示', '确定更改此课件信息?', function(r) {
		if (r) {
			$('#updCoursewareFrom').submit();
			return false;
		}
	});
}