...
$(function() {
	...
	$('#addExamForm').form({
		success : function(data) {
			if(data){
				...
				window.parent.openTab("考试管理",baseUrl + '/admin/T040/examManage.html');
	...
	$("#studyTimeStart").datetimebox("setValue", begintmStr);
	$("#studyTimeEnd").datetimebox("setValue", endtmStr);
	if(usersignupflg == 1){
		$("#tab").tabs("select", 0);
		$('#tab').tabs('getTab','用户/组').panel('options').tab.hide();
		$("#userSignupFlg").attr("checked",true);
	}else{
		$('#tab').tabs('getTab','用户/组').panel('options').tab.show();
	}
	if(passingagainflg == 1){
		$("#passingAgainFlg").attr("checked",true);
	}
	if(qsnrandomflg == 1){
		$("#qsnRandomFlg").attr("checked",true);
	}
	if(optionChaosFlg == 1){
		$("#optionChaosFlg").attr("checked",true);
	}
	if(scorePulic == 1){
		$("#scorePulic").attr("checked",true);
	}
	if(publishanswerflg == 1){
		$("#publishAnswerFlg").attr("checked",true);
	}
	if(isresultrank == 1){
		$("#isResultRank").attr("checked",true);
	}
	
	$("#examManual").combobox("setValue",examManual);
	$("#pagesize").combobox("setValue",pagesize);
	$("#selectFlg").combobox("setValue",selectFlg);
	$("#unityduration").combobox("setValue",unityduration);
	
	if(publishTimeStr != "1970-01-01 00:00"){
		$("#scorePulic").attr("checked",true);
		$("#resultPublishTime").datetimebox("setValue",publishTimeStr);
	}
	
	$("#coverPic").attr("src",$("#cover").val());
	...
	// 试卷列表
	$.ajax({
		url:baseUrl + "/admin/T040/findExamPapersSelected.html",
		data:{'ids': ids},
		type: 'post',
		dataType: 'json',
		success:function(result){
			$("#shijuanManage").datagrid({"data":result});
		}
	});
	// 证书列表
	$.ajax({
...
function addShiJuan(){
	if(flagShiJuan == 0){
		...
		$('#sjClassify').combotree({
			...
			animate : true,
			editable : false,// 不可编辑
			onHidePanel : getChildrensj,//获取子节点
...
function saveShiJuan(){
	$.messager.confirm('警告！', '修改试卷将会导致成绩丢失或正在考试的考生无法交卷，请慎重修改！', function(r) {
		if (r) {
			$("#updTypeFlag").val("1");
...
// 从考试中删除试卷
function delShiJuan(){
	$.messager.confirm('警告！', '删除试卷将会导致成绩丢失或正在考试的考生无法交卷，请慎重修改！', function(r) {
		if (r) {
			$("#updTypeFlag").val("1");
...
function validateFileType(fileName){
	...
	var checkType=false;
	// 判断类型
	checkType = exetendName=="png"||exetendName=="jpg"||exetendName=="gif"||exetendName=="jepg";
	// 返回文件类型的验证值
	return checkType;
}

function copyExam(){
    if(totalScore < parseInt($("#okrate").val())){
        alert("及格分不能大于总分！");
        return;
...
function changeTimeValidate(obj){
	var item=$(obj);
	if(item.val()=="1"){
		$("#resultPublishTime").datetimebox({required: true});
		//$("#resultPublishTime").datetimebox('setValue',CurentTime());
	}else{
		$("#resultPublishTime").datetimebox({required: false});
	}
}
