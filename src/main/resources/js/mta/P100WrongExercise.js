...
$(function() {
	...
	$.post(baseUrl + '/mta/P100/findUserWrongQsn.html',function(data){
		if(data.res != -1){
			loadSjDl(data);
		}else{
	...
	for(var index in dlData){
		...
		if( key in xtData){
			...
			for(var i=0;i<sjdata.length;i++){
				var item=sjdata[i];
				stNumHtml+='<li class="xuanxiangkaNumber_'+index+'" id="biao_'+item.qsnid+'" style="border-top: 1px solid #ccc;"><a href="'+baseUrl+'/mta/P100/goWrongExercise.html#dl_qsn_'+qsnIndex+'">'+qsnIndex+'</a></li>';
...
function loadQsnInfo(qindex,qsnData,isSuiJi){
	...
	var gdQsnHtml='<tr id="dl_qsn_'+qindex+'" suiji="0" jieda="'+qsnData.jieda+'" class="shiti" shitiId="'+qsnData.qsnid+'" shitiType="'+qsnData.shititypeid+'"><th colspan="3" align="left">第'+qindex+'题:<span class="title">'+qsnData.title+'</span> </th>'
		...
		for(var ki in kongsData){
			...
			for(var kai=0;kai<kongArray.length;kai++){
				if(kai==0){
					...
					gdQsnHtml += '<input type="text" stId="'+qsnData.qsnid+'" onchange="tiankong(this)" class="kong" name="tk_'+qindex+'_daan" id="tabletextfield" />';
				}else{
				    if(kongArray[kai] != ''){
...
//		alert(answerItems.length);
		...
		var kongsData=eval("("+rightAnswer+")");
		var wStatus=0;
		for(var i=0;i<answerItems.length;i++){
			var ksdaan=$(answerItems[i]).val();
			var rkey=i+1;
			var rDaanArray=kongsData[rkey+""].kong.split("#");
