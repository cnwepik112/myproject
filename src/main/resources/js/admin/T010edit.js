...
$(function() {
	...
	//初始化试题提交表单
	$('#qsnForm').form({
		url : baseUrl + "/admin/T010/saveRadioQsn.html",
		onSubmit : function() {
...
function addQsnAnswerLi() {
		$.post(baseUrl+ "/admin/T010/findRadioQsn.html",{qsnid:rqsnid},function(data){
			...
			for(var i in xxItems){
				//循环答案并向OL中添加答案HTML
				var liHtml = "<li>"
					+ "<input type='radio' name='answer' id='answer_radio_"+num+"'/>答案&nbsp;&nbsp;<a class='blue_color_a' href='javascript:;' onclick='removeAnswerChild(this)'>删除</a>"
...
function addAnswerChild() {
	...
	var liHtml = "<li>"
			+ "<input type='radio' name='answer'/>答案&nbsp;&nbsp;<a class='blue_color_a' href='javascript:;' onclick='removeAnswerChild(this)'>删除</a>"

			+ "&nbsp;&nbsp;&nbsp;&nbsp;" + "<span id='answer_" + nextNum
			+ "_content' style='color:red;'></span><br/>"
			+ "<textarea id='answer_" + nextNum + "' ueid='answer_" + nextNum
			+ "' name='content' style='width:90%;height:50px;' > </textarea>"
			+ "</li>";
	$("#answerOl").append(liHtml);
